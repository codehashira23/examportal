...
export const GlareCard = ({
  ...
  const backgroundStyle = {
    ...
    "--shade":
      "radial-gradient( farthest-corner circle at var(--m-x) var(--m-y),rgba(255,255,255,0.05) 12%,rgba(255,255,255,0.08) 20%,rgba(255,255,255,0.15) 120% ) var(--bg-x) var(--bg-y)/300% no-repeat",
  ...
  if (refElement.current) {
    refElement.current.style.setProperty("--m-x", "30%");
    refElement.current.style.setProperty("--bg-x", "30%");
  }
  ...
  return (
    <div
      ...
      onPointerMove={(event) => {
        ...
        const { background, rotate, glare } = state.current;
        background.x = Math.min(50, 30 + percentage.x / 5);
        background.y = 50 + percentage.y / 3 - 16.67;
        rotate.x = -(delta.x / 3.5);
        rotate.y = delta.y / 2;
        rotate.x *= rotateFactor;
        rotate.y *= rotateFactor;
        glare.x = Math.min(50, 20 + percentage.x / 2.5);
